...
(function (root, factory) {
    ...
    */
     ...
     */
    Countly.initialize_surveys = function(type, surveys) {
        ...
        var url = "";
        if(type === "nps") {
            url += Countly.url + '/o/surveys/nps/widget';
        }

        sendXmlHttpRequest(url, { widget_ids: surveys.join()}, function(err, params, responseText){
            if(err) {
    ...
    * example configuration: {"popups":["5b21581b967c4850a7818617"]}
    **/
    Countly.initialize_nps_surveys = function(params) {
        if (offlineMode) {
            log("Cannot enable feedback in offline mode");
        }
        else {
            ...
            if (surveys.length > 0) {
                ...
                Countly.initialize_surveys("nps", surveys);
            } else {
    ...
    **/
    ...
    function processNpsSurveys(currSurvey) {
        try {
            var wrapper = document.createElement("div");
            wrapper.className = 'countly-surveys-wrapper';
            wrapper.id = 'countly-surveys-wrapper-' + currSurvey._id;

            var iframe = document.createElement("iframe");
            iframe.name = "countly-surveys-iframe";
            iframe.id = "countly-surveys-iframe";
            iframe.src = Countly.url + "/surveys/widget/nps?widget_id=" + currSurvey._id + "&app_key=" + Countly.app_key + '&device_id=' + Countly.device_id + '&sdk_version=' + SDK_VERSION + '&app_version=' + Countly.app_version + '&origin=' + window.origin;

            document.body.appendChild(wrapper);

            // iframe.onload = function() {
            //     document.getElementById('countly-surveys-wrapper-' + currSurvey._id).style.display = "none";
            ...
            document.getElementById('countly-surveys-wrapper-' + currSurvey._id).style.display = "block";
