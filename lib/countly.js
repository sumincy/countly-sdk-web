...
(function (root, factory) {
    ...
    */
     ...
     * @param {array=} - optional - widget ids array
     */
    Countly.initialize_feedback_popups = function(enableWidgets) {
        if (!enableWidgets) {
    ...
    /**
     * Prepare widgets according to current options
     * @param {array=} - surveys - widget ids array
     */
    Countly.initialize_surveys = function(surveys) {
        loadCSS(Countly.url + '/surveys/stylesheets/countly-surveys.css');
        if (!surveys && !surveys.length) {
            log("Please pass nps surveys ids");
            return;
        }
        ...
        sendXmlHttpRequest(Countly.url + '/o/feedback/multiple-widgets-by-id', { widgets: JSON.stringify(surveys)}, function(err, params, responseText){
            if(err) {
                log("Error occured while fetching nps surveys", err);
                return;
            }
            ...
            try {
                ...
                for (var i = 0; i < widgets.length; i++) {
                    if (widgets[i].status) {
                        ...
                        processNpsSurveys(widgets[i], true);
                    }
                }

            } catch (error) {
                log("Error before processing widget:  " + error);
            }
        });
    };
    ...
    **/
    Countly.enable_feedback = function(params) {
        if (offlineMode) {
            ...
            if (enableWidgets.length > 0) {
                ...
                Countly.initialize_feedback_popups(enableWidgets);
            } else {
    ...
    /**
    * Enable NPS surveys
    * @param {object=} configuration - required - Pass in nps surveys to show
    * example configuration: {"popups":["5b21581b967c4850a7818617"]}
    **/
    Countly.insert_nps_surveys = function(params) {
        if (offlineMode) {
            log("Cannot enable feedback in offline mode");
        }
        else {
            var surveys = params.surveys;

            if (surveys.length > 0) {
                document.body.insertAdjacentHTML('beforeend', '<div id="cfbg"></div>');
                Countly.initialize_surveys(surveys);
            } else {
                log("You should provide at least one widget id as param.");
            }
        }
    };
    ...
    **/
    ...
    function processNpsSurveys(currSurvey, hasSticker) {
        try {
            var wrapper = document.createElement("div");
            wrapper.className = 'countly-iframe-wrapper';
            wrapper.id = 'countly-iframe-wrapper-' + currSurvey._id;

            var iframe = document.createElement("iframe");
            iframe.name = "countly-feedback-iframe";
            iframe.id = "countly-feedback-iframe";
            iframe.src = Countly.url + "/nps?widget_id=" + currSurvey._id + "&app_key=" + Countly.app_key + '&device_id=' + Countly.device_id + '&sdk_version=' + SDK_VERSION + '&app_version=' + Countly.app_version + '&origin=' + window.origin;

            document.body.appendChild(wrapper);

            // iframe.onload = function() {
            //     document.getElementById('countly-iframe-wrapper-' + currSurvey._id).style.display = "none";
            //     document.getElementById('cfbg').style.display = "none";
            // }
            ...
            if(hasSticker) {
                ...
                var stickerText = document.createElement("span");
                stickerText.innerText = "NPS Survey";

                var sticker = document.createElement("div");
                sticker.style.color = "black";
                sticker.style.backgroundColor = "pink";
                sticker.className = 'countly-feedback-sticker ' + "mright-s";
                sticker.id = 'countly-feedback-sticker-' + currSurvey._id;
                svgIcon.appendChild(svgPath);
                sticker.appendChild(svgIcon);
                sticker.appendChild(stickerText);
                document.body.appendChild(sticker);
                ...
                var smileySvg = document.getElementById("smileyPathInStickerSvg");
                if(smileySvg) {
                    smileySvg.style.fill = "black";
                }

                add_event(document.getElementById('countly-feedback-sticker-' + currSurvey._id), 'click', function(){
                    document.getElementById('countly-iframe-wrapper-' + currSurvey._id).style.display = "block";
                    document.getElementById('cfbg').style.display = "block";
                });
            }
            else {
            ...
            log("Somethings went wrong while inserting nps survey: " + e);
        }
    }
    ...
    function getMetrics() {
        var metrics = JSON.parse(JSON.stringify(Countly.metrics || {}));

        //getting app version
        metrics._app_version = metrics._app_version || Countly.app_version;
